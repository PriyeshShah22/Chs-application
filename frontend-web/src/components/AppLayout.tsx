...
import { CampaignRounded, DashboardRounded, DarkModeRounded, GroupsRounded, LightModeRounded, LogoutRounded, PaymentsRounded, RecordVoiceOverRounded, ReportProblemRounded, SettingsRounded, ShieldRounded, SpaceDashboardRounded } from "@mui/icons-material";
...
export default function AppLayout() {
  ...
  const mobileItems = managementItem ? [...primary, managementItem] : primary;
  ...
  return <Box sx={{ minHeight: "100vh", bgcolor: "background.default", pb: { xs: 10, md: 0 } }}><CssBaseline />
    ...
    {mobile && <Box component="nav" aria-label="Mobile navigation" sx={{ position: "fixed", left: 10, right: 10, bottom: 10, zIndex: 1300, bgcolor: "#173F35", color: "white", borderRadius: 3, p: .75, boxShadow: "0 18px 50px rgba(23,63,53,.3)", display: "grid", gridTemplateColumns: `repeat(${mobileItems.length},1fr)` }}>{mobileItems.map((item) => <IconButton key={item.to} aria-label={t(item.label)} onClick={() => navigate(item.to)} sx={{ color: active(item.to) ? "#F4B860" : "rgba(255,255,255,.65)", flexDirection: "column", borderRadius: 2, fontSize: 10, gap: .2, minWidth: 0 }}>{item.icon}<span>{t(item.label)}</span></IconButton>)}</Box>}
  </Box>;
}
