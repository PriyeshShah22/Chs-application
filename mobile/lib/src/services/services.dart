...
class ComplaintListController
  ...
  Future<void> refresh() async {
    ...
    try {
      final res =
          await ref.read(apiClientProvider).dio.get('/complaints/?limit=100');
  ...
  Future<bool> create(
    ...
    try {
      final res =
          await ref.read(apiClientProvider).dio.post('/complaints/', data: {
        'title': title,
...
final complaintListProvider =
    StateNotifierProvider<ComplaintListController, AsyncValue<List<Complaint>>>(
...
class BillListController extends StateNotifier<AsyncValue<List<Bill>>> {
  ...
  Future<void> refresh() async {
    ...
    try {
      final res =
          await ref.read(apiClientProvider).dio.get('/bills/?limit=100');
      final list = (res.data as List)
...
final billListProvider =
    StateNotifierProvider<BillListController, AsyncValue<List<Bill>>>(
...
class NoticeListController extends StateNotifier<AsyncValue<List<Notice>>> {
  ...
  Future<void> refresh() async {
    ...
    try {
      ...
      final list = (res.data as List)
          .map((j) => Notice.fromJson(j as Map<String, dynamic>))
          .toList();
      state = AsyncValue.data(list);
    } catch (e, s) {
      state = AsyncValue.error(e, s);
...
final noticeListProvider =
    StateNotifierProvider<NoticeListController, AsyncValue<List<Notice>>>(
...
class VisitorListController extends StateNotifier<AsyncValue<List<Visitor>>> {
  ...
  Future<void> refresh() async {
    ...
    try {
      final res =
          await ref.read(apiClientProvider).dio.get('/visitors/?limit=100');
      final list = (res.data as List)
          .map((j) => Visitor.fromJson(j as Map<String, dynamic>))
          .toList();
      state = AsyncValue.data(list);
    } catch (e, s) {
      state = AsyncValue.error(e, s);
  ...
  Future<bool> register(
    ...
    try {
      final res =
          await ref.read(apiClientProvider).dio.post('/visitors/', data: {
        'society_id': 1,
        'flat_id': 1,
        'name': name,
        'phone': phone,
  ...
  Future<bool> action(int visitorId, String kind) async {
    try {
      final res = await ref
          .read(apiClientProvider)
          .dio
          .post('/visitors/$visitorId/action', data: {
        'action': kind,
      });
      if ((res.statusCode ?? 0) >= 200 && (res.statusCode ?? 0) < 300) {
        await refresh();
...
final visitorListProvider =
    StateNotifierProvider<VisitorListController, AsyncValue<List<Visitor>>>(
